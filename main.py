...
class NfaToDfa:
    ...
    def __init__(self):
        self.nfa_matrix = np.matrix(['main', 'end', 'value'])
        self.dfa_matrix = np.matrix(['main', 'end', 'value'])
        self.nfa_status_matrix = np.matrix(['main', 'is_end'])
        self.dfa_status_matrix = np.matrix(['main', 'is_end'])
        self.directions = 0
        self.nfa_count = 1
        self.dfa_count = 1
    ...
    def next_move(self):
        for i in self.dfa_matrix:
            if i.item(0) == 'main' or i.item(1) == 'None':
                continue
    ...
    def get_q0_move(self):
        ...
        is_end = 'True' if self.nfa_status_matrix.item(int(find_q0[1][0]), 1) == 'True' else 'False'
        value_0 = self.list_to_str(value_0)
        value_1 = self.list_to_str(value_1)
